(function ($) {
  ...
  $(function () {
    ...
    $(".lightbox-wrapper").each(function () {
      if (
        !$('.navbar .navbar-nav .nav-link[href^="#' + this.id + '"]').length
      ) {
        $(this).hide();
      }
    });
      ...
      var initPercentageElement = function () {
        $("#resume .skills-section .single-skill").each(function () {
          var percentage = Math.min(
            100,
            Math.max(0, $(this).data("percentage"))
          );
          var barWidth = $(this).find(".skill-progress").outerWidth(true);
          var percentageElementOffset =
            barWidth - barWidth * (percentage / 100);
          $(this)
            .find(".skill-percentage")
            .text(percentage + "%")
            .css("margin-right", percentageElementOffset);
          $(this)
            .find(".progress-bar")
